...
bool CCCCStake::InitFromTxIn(const CTxIn& txin)
{
...
bool CCCCStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CCCCStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CCCCStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CCCCStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CCCCStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CCCCStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
CDataStream CCCCStake::GetUniqueness() const
{
    //The unique identifier for a CCC stake is the outpoint
...
CBlockIndex* CCCCStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CCCCStake::ContextCheck(int nHeight, uint32_t nTime)
{
