...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "ccc.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "cccd.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 7979, 17980));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all ccc specific functionality (Masternodes, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-minstakesplit=<amt>", strprintf(_("Minimum positive amount (in CCC) allowed by GUI and RPC for the stake split threshold (default: %s)"),
...
/*  // CCC
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:7979"));
...
*/
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 7980, 27980));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2020-%i The SUDA developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The Litecoin Core developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i The CeilingCatCoin developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that ccc is running in a usable environment with all
...
void InitLogging()
{
    ...
    // Disable unsupported features of this CCC version
    g_logger->DisableCategory(BCLog::MASTERNODE);
    g_logger->DisableCategory(BCLog::MNBUDGET);
    g_logger->DisableCategory(BCLog::STAKING);
...
bool AppInit2()
{
 ...
 /*   if (GetBoolArg("-masternode", false) && !GetBoolArg("-listen", true))
    ...
    // Sanity check
    if (!InitSanityCheck())
        return UIError(_("Initialization sanity check failed. ccc is shutting down."));
...
#endif
    // Make sure only a single ccc process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return UIError(strprintf(_("Cannot obtain a lock on data directory %s. ccc is probably already running."), strDataDir));
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                // ccc: load previous sessions sporks if we have them.
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of ccc") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart ccc to complete") << "\n";
        ...
        if (GetBoolArg("-upgradewallet", fFirstRun && !fLegacyWallet)) {
            if (prev_version <= FEATURE_PRE_ccc && pwalletMain->IsLocked()) {
        ...
        if (fFirstRun) {
            if (!fLegacyWallet) {
                ...
                pwalletMain->SetMaxVersion(FEATURE_PRE_ccc);
            }
    ...
    if (readResult == CMasternodeDB::FileError)
        LogPrintf("Missing mncache file - mncache.dat, will try to recreate\n");
    ...
    if (readResult2 == CBudgetDB::FileError)
        LogPrintf("Missing budget.dat, will try to recreate\n");
    ...
    if (readResult3 == CMasternodePaymentDB::FileError)
        LogPrintf("Missing mnpayments.dat, will try to recreate\n");
    ...
    // CCC: unsupported in this version
    fMasterNode = false;//GetBoolArg("-masternode", false);
    ...
    // CCC: disabled
    strBudgetMode = false;//GetArg("-budgetvotemode", "auto");
/*
