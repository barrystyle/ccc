...
#if defined(NDEBUG)
#error "ccc cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uccc) are considered zero fee (for relaying, mining and transaction creation)
...
bool RecalculateCCCSupply(int nHeightStart)
{
	AssertLockHeld(cs_main);
	...
	uiInterface.ShowProgress(_("Recalculating CCC supply..."), 0);
	while (true) {
		if (pindex->nHeight % 1000 == 0) {
			LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
			int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainHeight - nHeightStart))));
			uiInterface.ShowProgress(_("Recalculating CCC supply..."), percent);
		}
...
{
    CAmount mnSubsidy = 0;      // no MN payment in this version
...
{
    CAmount nDevPayment = 0;    // no DEV payment in this version
...
{
    int64_t nMNCollateral = 999999999;
...
{
    util::ThreadRename("ccc-scriptch");
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> cccInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                cccInputs.push_back(stakeIn);
        }
        const bool hasCCCInputs = !cccInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasCCCInputs) {
                    ...
                    for (const CTxIn& cccIn : cccInputs)
                        if(cccIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasCCCInputs) {
                            for (const CTxIn& stakeIn : cccInputs)
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // ccc: We use certain sporks during IBD, so check to see if they are
