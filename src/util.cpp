...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\ccc
// Windows >= Vista: C:\Users\Username\AppData\Roaming\ccc
// Mac: ~/Library/Application Support/ccc
// Unix: ~/.ccc
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CCC";
#else
    fs::path pathRet;
...
{
    fs::path pathConfigFile(GetArg("-conf", "ccc.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty ccc.conf if it does not exist
        FILE* configFile = fsbridge::fopen(GetConfigFile(), "a");
        if (configFile != NULL)
        {
            std::string strHeader = "addnode=168.100.10.230:7979\n"
                                    "addnode=162.33.179.114:7979\n"
                                    "addnode=45.61.136.133:7979\n"
                                    "addnode=45.61.139.165:7979\n"
                                    "addnode=45.61.137.253:7979\n";
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override ccc.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", "cccd.pid"));
