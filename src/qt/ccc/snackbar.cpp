...
#include "qt/ccc/forms/ui_snackbar.h"
#include "qt/ccc/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(cccGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &cccGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
