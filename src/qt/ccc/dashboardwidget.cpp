...
#include "qt/ccc/dashboardwidget.h"
#include "qt/ccc/forms/ui_dashboardwidget.h"
#include "qt/ccc/sendconfirmdialog.h"
#include "qt/ccc/txrow.h"
...
DashboardWidget::DashboardWidget(cccGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareCCC, "square-chart-ccc");
    setCssProperty(ui->labelCCC, "text-chart-ccc");
    ...
    setCssProperty(ui->labelAmountCCC, "text-stake-ccc-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &cccGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &cccGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &cccGUI::openFAQ);
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            ccc = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalCCC += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesCCC.append(ccc);

        int max = std::max(ccc, ccc);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesCCC);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalCCC > 0) {
        setCssProperty(ui->labelAmountCCC, "text-stake-ccc");
    } else {
        setCssProperty(ui->labelAmountCCC, "text-stake-ccc-disable");
    }
    forceUpdateStyle({ui->labelAmountCCC});
    ui->labelAmountCCC->setText(GUIUtil::formatBalance(chartData->totalCCC, nDisplayUnit));
