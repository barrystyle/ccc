...
#include "qt/ccc/forms/ui_send.h"
#include "qt/ccc/addnewcontactdialog.h"
#include "qt/ccc/qtutils.h"
#include "qt/ccc/sendchangeaddressdialog.h"
#include "qt/ccc/optionbutton.h"
#include "qt/ccc/sendconfirmdialog.h"
#include "qt/ccc/myaddressrow.h"
#include "qt/ccc/guitransactionsutils.h"
...
SendWidget::SendWidget(cccGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-ccc");
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        CTxDestination cccAdd = DecodeDestination(address.toStdString());
        if (walletModel->isMine(cccAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(cccAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
