...
#include "qt/ccc/forms/ui_addresseswidget.h"
#include "qt/ccc/addresslabelrow.h"
#include "qt/ccc/addnewaddressdialog.h"
#include "qt/ccc/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(cccGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        CTxDestination cccAdd = DecodeDestination(address.toUtf8().constData(), isStakingAddress);
        ...
        if (walletModel->updateAddressBookLabels(cccAdd, label.toUtf8().constData(),
