...
#include "qt/ccc/forms/ui_masternodeswidget.h"

#include "qt/ccc/qtutils.h"
#include "qt/ccc/mnrow.h"
#include "qt/ccc/mninfodialog.h"
#include "qt/ccc/masternodewizarddialog.h"
...
MasterNodesWidget::MasterNodesWidget(cccGUI *parent) :
    PWidget(parent),
    ...
    if (fs::exists(pathBootstrap)) {
        ...
        if (lineCopy.size() == 0) {
            lineCopy = "# Masternode config file\n"
                                    ...
                                    "# Example: mn1 127.0.0.2:7979 93HaYBVUCYjEMeeH1Y4sBGLALQZE1Yc1K64xiqgX37tGBDQL8Xg 2bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c 0\n";
        }
...
void MasterNodesWidget::onCreateMNClicked()
{
    ...
    if (walletModel->getBalance() <= GetMNCollateral(chainActive.Height()) * COIN) {
        inform(tr("Not enough balance to create a masternode, %1 CCC required.").arg(GetMNCollateral(chainActive.Height())));
        return;
    }
    showHideOp(true);
