...
*Setup instructions for a gitian build of ccc using a VM or physical system.*

Gitian is the deterministic build process that is used to build the ccc
...
to the ccc GitHub release page.
...
- [Building ccc](#building-ccc-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/ccc-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/ccc-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/ccc-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/ccc-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/ccc-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/ccc-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/ccc-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@ceilingcatcoin.com) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/ccc-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/ccc-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/ccc-osx-*-build.assert
```
